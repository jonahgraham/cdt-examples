...
import com.kichwacoders.cdt.dsf.tutorial.timers.AlarmService.TriggerDMContext;
...
public class TriggerCellModifier implements ICellModifier {
    ...
    @ThreadSafe
    private ServiceTracker<?,AlarmService> fServiceTracker;
    ...
    private Shell getShell() {
        if (TutorialPlugin.getDefault().getWorkbench().getActiveWorkbenchWindow() != null) {
            return TutorialPlugin.getDefault().getWorkbench().getActiveWorkbenchWindow().getShell();
        }
        return null;
    }
    ...
    private synchronized AlarmService getService(TriggerDMContext dmc) {
        ...
        if (fServiceTracker == null) {
            try {
                fServiceTracker = new ServiceTracker<>(
                    TutorialPlugin.getBundleContext(), 
                    TutorialPlugin.getBundleContext().createFilter(serviceId), 
                    null);
        ...
        // Get the service.
        return fServiceTracker.getService();
    }
    ...
    private class GetValueQuery extends Query<Integer> {
        ...
        protected void execute(final DataRequestMonitor<Integer> rm) {
            ...
            if (session == null) {
                rm.setStatus(new Status(IStatus.ERROR, TutorialPlugin.PLUGIN_ID, IDsfStatusConstants.INVALID_STATE, "Debug session already shut down.", null)); //$NON-NLS-1$
                rm.done();
                return;
            }
            ...
            if (service == null) {
                rm.setStatus(new Status(IStatus.ERROR, TutorialPlugin.PLUGIN_ID, IDsfStatusConstants.INVALID_STATE, 
            ...
            if (value == -1) {
                rm.setStatus(new Status(IStatus.ERROR, TutorialPlugin.PLUGIN_ID, IDsfStatusConstants.INVALID_HANDLE, 
